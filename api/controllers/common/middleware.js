  ...
  const checkAdminRoleAccess = function(modulePermissions, opts) {
    ...
    return function(req, res, next) {
      ...
      if (admin.roleInfo.isSuperAdmin) {
        ...
        let hasExtraPrivilege = true;
        if (opts) {
          if (opts.hasOwnProperty('canCancelBooking')) {
            hasExtraPrivilege = admin.roleInfo.roleId.canCancelBooking;
          } else
          if (opts.hasOwnProperty('canEndTrip')) {
            hasExtraPrivilege = admin.roleInfo.roleId.canCancelBooking;
          }
        }
